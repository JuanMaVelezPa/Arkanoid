...
> **Date:** 2026-09-18
> **Objective:** Add five brick-pattern levels with auto-advance, a LEVEL HUD, Esc pause, and keys 1-5 to jump level while paused.
...
- Esc only while `playing` (glued or in flight) sets `paused` and shows the existing HTML overlay: title `PAUSED`, msg `Esc, Space or click to resume. Keys 1-5 change level.`
- Esc, Space, or click while `paused` resumes `playing` and hides the overlay. That press does not launch a glued ball.
...
4. Add `paused`. Esc in `playing` sets `state` to `'paused'`, shows overlay copy from Scope, skips `update` movement. Esc / Space / click while paused returns to `'playing'` and `hideOverlay`, and does not serve. Ignore Esc on Start / Win / Lose. Manual test: mid-game Esc freezes ball and paddle; Space resumes without launching a glued ball; a second Space serves if glued.

5. While paused, keys `1`..`5` call `loadLevel(n)`, hide overlay, set `playing`. Keep score and lives. Manual test: break a few bricks, Esc, press 3; HUD says 3, pyramid is intact, ball is glued, score unchanged.
...
- [ ] HUD shows `LEVEL` and the current number `1`..`5`. `python3 -m http.server 8000` still loads with no new 404s.
- [ ] Start (first load or after Win/Lose) always begins level 1, score 0, lives 3, ball glued. Pattern is the full 13x6.
...
- [ ] Esc during `playing` shows `PAUSED` and the agreed msg, and freezes paddle, ball, bricks, and explosions.
- [ ] Esc, Space, or click while paused resumes play and does not serve a glued ball on that same press.
- [ ] Esc does nothing on Start, Win, and Lose.
...
- **Yes:** Esc pause only in `playing`. Resume with Esc, Space, or click. Space on pause does not serve.
