...
- [x] Breaking a brick hides that brick immediately and plays the 4-frame atlas explosion in the same `x, y, w, h`.
- [x] The explosion lasts 150ms (`EXPLOSION_DURATION`) and uses all 4 frames in order.
- [x] Row colors `red`, `yellow`, `cyan`, `magenta`, `hotpink`, `green` each use `EXPLOSION_FRAMES` for that color.
- [x] Explosions draw above remaining bricks and below paddle and ball.
- [x] Several explosions can play at once.
- [x] Win overlay still appears on the same break that clears the last brick. No delay for animations.
- [x] Explosions keep advancing while the Win/Lose overlay is visible. A new game from Start shows none of the old frames.
- [x] `spritesheet.js` is unchanged. No new files, no `style.css` change, no extra audio.
- [x] Breaking a brick still adds exactly 10 points and still plays `break-sound.mp3`.
